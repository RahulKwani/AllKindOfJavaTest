...
public class GenToken {
  ...
  public static void mainOld(String[] args) throws IOException {
    ...
    FileInputStream serviceAccount =
        new FileInputStream(
            "/Users/rahul/Documents/My_Home/PerProj"
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      while ((line = bufferedReader.readLine()) != null) {
        ...
        if (!line.isEmpty()) inputDataList.add(line);
        // numLines++;
      }
      ...
      for (String api : apiList) {
        sortedApiMap.put(api.substring(0, api.indexOf(",")), api.substring(api.indexOf(",") + 1));
      }
      ...
      while (it.hasNext()) {
        ...
        if (apiCountMap.get(apiName) > 1) {
          for (String str : inputDataList) {
            if (str.contains(apiName + "," + sortedApiMap.get(apiName))) {
